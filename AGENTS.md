...
Kali Linux provisioning scripts under `src/scripts/`: Omarchy-style per-app
install/config scripts, orchestrated by `master.sh`, `run-install.sh`, and
`run-config.sh`. Kali is Debian-based, so apt commands and repo setup are very
similar to Ubuntu, but Kali-specific security tooling (1Password, ClamAV,
OWASP ZAP, SecLists, PayloadsAllTheThings, nmap, etc.) is preserved.
...
# or
find src/scripts scripts -name '*.sh' -print0 | xargs -0 shellcheck -x
```

Kali scripts use `.shellcheckrc` conventions (`external-sources=true`,
...
`.github/workflows/test-runner.yaml` runs four jobs on a `kalilinux/kali-rolling`
container:
...
| `src/scripts/install/apps/1password.sh`                     | 1Password desktop app and CLI (Kali-specific)           |
